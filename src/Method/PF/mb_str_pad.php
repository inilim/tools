...
function mb_str_pad(string $string, int $length, string $pad_string = ' ', int $pad_type = \STR_PAD_RIGHT, ?string $encoding = null): string
{
    ...
    if (!\in_array($pad_type, [\STR_PAD_RIGHT, \STR_PAD_LEFT, \STR_PAD_BOTH], true)) {
        throw new \Error('PF::mb_str_pad(): Argument #4 ($pad_type) must be STR_PAD_LEFT, STR_PAD_RIGHT, or STR_PAD_BOTH');
    }
    ...
    try {
        ...
        throw new \Error(\sprintf('PF::mb_str_pad(): Argument #5 ($encoding) must be a valid encoding, "%s" given', $encoding));
    }
    ...
    if (\mb_strlen($pad_string, $encoding) <= 0) {
        throw new \Error('PF::mb_str_pad(): Argument #3 ($pad_string) must be a non-empty string');
    }

    $paddingRequired = (int)($length - \mb_strlen($string, $encoding));
