 ...
 * @return null|array
 */
function excelConvertSheetToDocXml($pathToFileOrZip, string $sheetId): ?array
{
    ...
    $anonObj = new class(
        ...
        function __invoke(): ?array
        {
            ...
            $pathToFileTmp = $this->saveResourceAndInfo($resourceSheet, $aInfo);
            ...
            unset($resourceSheet);
            $sheetDoc = new \XMLReader;
            $sheetDoc->open($pathToFileTmp);
            $pathToFileTmp = $this->saveResourceAndInfo($resourceSharedStrings, $aInfo);
            if ($pathToFileTmp === null) {
                return null;
            }
            dde($sheetDoc->baseURI);
            \fclose($resourceSharedStrings);
            unset($resourceSharedStrings);
            $sharedStrsDoc = new \XMLReader;
            $sharedStrsDoc->open($pathToFileTmp);
            ...
            return [
                ...
                'sheet' => [
                    'id'   => $this->id,
                    'xml'  => $sheetDoc,
                    'file' => $sheetDoc->baseURI,
                ],
                'shared_strings' => [
                    'xml'  => $sharedStrsDoc,
                    'file' => $sharedStrsDoc->baseURI,
                ]
            ];
        }
