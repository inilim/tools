...
namespace Inilim\Tool\Method\Time{function unixMsFromGlobals():int{$rtf=\Inilim\Tool\Method\LarArr\float($_SERVER,'REQUEST_TIME_FLOAT');return \intval(\intval($rtf).'000')+\intval(\substr((string) $rtf,11,3));}}namespace Inilim\Tool\Method\Check{if(!\Inilim\Tool\Check::__definedIfNot('php80')){
    ...
    }if(!\Inilim\Tool\LarArr::__definedIfNot('float')){
    function float($array,$key,?float $default=null):float{$value=\Inilim\Tool\Method\LarArr\get($array,$key,$default);if(!\is_float($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be a float, %s found.',$key,\gettype($value)));}return $value;}
