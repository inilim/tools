...
declare(strict_types=1);namespace Inilim\Tool\Method\Time{function unixMsFromGlobals():int{$rtf=\Inilim\Tool\Method\Arr\float($_SERVER,'REQUEST_TIME_FLOAT');return \intval(\intval($rtf).'000')+\intval(\substr((string) $rtf,11,3));}}namespace Inilim\Tool\Method\Arr{if(!\Inilim\Tool\Arr::__definedIfNot('accessible')){
    ...
    }if(!\Inilim\Tool\Arr::__definedIfNot('float')){
    function float($array,$key,?float $default=null):float{$value=\Inilim\Tool\Method\Arr\get($array,$key,$default);if(!\is_float($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be a float, %s found.',$key,\gettype($value)));}return $value;}
