...
namespace Inilim\Tool\Method\Exp{function fgcSendJsonPost(string $url,$body,array $headers=[],array $ctxOptions=[]){if(\is_array($body)){$body=\json_encode($body);}else{\Inilim\Tool\Method\Assert\json($body);}$headers[]='content-type: application/json';return \Inilim\Tool\Method\Exp\fgcSend($url,'POST',$body,$headers,$ctxOptions);}if(!\Inilim\Tool\Exp::__definedIfNot('fgcSend')){
    ...
    function getViaArray(array $params){return \Inilim\Tool\Method\File\get($params['pathToFile'],$params['offset']?? 0,$params['length']?? null,$params['useIncludePath']?? false,$params['throw']?? false,$params['context']?? null,$params['contextParams']?? null);}
    ...
    function getCollectionThrowable(string $message = '', int $code = 0, ?int $line = null, ?string $file = null, ?\Throwable $previous = null)
{
    ...
    }if(!\Inilim\Tool\Assert::__definedIfNot('json')){
    function json($value,string $message=''){\Inilim\Tool\Method\Assert\string($value);if(!\Inilim\Tool\Method\Check\isJson($value)){throw new \InvalidArgumentException(\sprintf($message?:'Expected an json. Got: "%s"',$value));}}
