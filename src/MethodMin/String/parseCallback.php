...
namespace Inilim\Tool\Method\String{function parseCallback(string $callback,?string $default=null):array{if(\Inilim\Tool\Method\String\contains($callback,"@anonymous\x00")){if(\Inilim\Tool\Method\String\substrCount($callback,'@')>1){return[\Inilim\Tool\Method\String\beforeLast($callback,'@'),\Inilim\Tool\Method\String\afterLast($callback,'@')];}return[$callback,$default];}return \Inilim\Tool\Method\String\contains($callback,'@')?\explode('@',$callback,2):[$callback,$default];}if(!\Inilim\Tool\Str::__definedIfNot('_contains')){
    ...
    function beforeLast(string $subject,string $search):string{if($search===''){return $subject;}$pos=\mb_strrpos($subject,$search);if($pos===false){return $subject;}return \Inilim\Tool\Method\String\substr($subject,0,$pos);}
