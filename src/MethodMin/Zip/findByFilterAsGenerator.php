...
namespace Inilim\Tool\Method\Zip{function findByFilterAsGenerator($pathToFileOrZip,callable $predicate,$valueBreak=null):\Generator{foreach(\Inilim\Tool\Method\Zip\scanAsGenerator($pathToFileOrZip)as $fileItem){$v=$predicate($fileItem);if($v===$valueBreak){return;}if($v){yield $fileItem;}}}if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
