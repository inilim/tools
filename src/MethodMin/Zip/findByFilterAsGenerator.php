...
namespace Inilim\Tool\Method\Zip{function findByFilterAsGenerator($pathToFileOrZip,callable $predicate,$valueBreak=null):\Generator{foreach(\Inilim\Tool\Method\Zip\scanAsGenerator($pathToFileOrZip)as $stat){$t=$stat;$v=$predicate($t);if($v===$valueBreak){return;}if($v===true){yield $stat;}}}if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
