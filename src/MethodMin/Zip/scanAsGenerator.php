...
namespace Inilim\Tool\Method\Zip{function scanAsGenerator($pathToFileOrZip):\Generator{$zip=\Inilim\Tool\Method\Zip\getObjFrom($pathToFileOrZip);for($i=0;$i<$zip -> numFiles;$i++){$ri=$zip -> statIndex($i,\ZipArchive :: FL_UNCHANGED);if($ri===false){continue;}yield $ri;}}if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
