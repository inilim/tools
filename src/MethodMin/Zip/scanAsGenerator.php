...
namespace Inilim\Tool\Method\Zip{function scanAsGenerator($zip):\Generator{\Inilim\Tool\Method\Assert\extPhp('zip');if(\is_string($zip)){$zip=\Inilim\Tool\Method\Zip\open($zip,\ZipArchive :: RDONLY);if(!$zip){throw new \Exception(\sprintf('File "%s", not open',$zip));}}elseif($zip -> filename===''){throw new \Exception('Uninitialized zip');}for($i=0;$i<$zip -> numFiles;$i++){$ri=$zip -> statIndex($i);if($ri===false){continue;}yield $ri;}}if(!\Inilim\Tool\Zip::__definedIfNot('open')){
