...
namespace Inilim\Tool\Method\Zip{function findAllByCallable($pathToFileOrZip,callable $predicate):array{$results=[];foreach(\Inilim\Tool\Method\Zip\scanAsGenerator($pathToFileOrZip)as $stat){$t=$stat;if($predicate($t)===true){$results[]=$stat;}}return $results;}if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
