...
namespace Inilim\Tool\Method\Zip{function findFirstByCallable($pathToFileOrZip,callable $predicate):?array{$gen=\Inilim\Tool\Method\Zip\scanAsGenerator($pathToFileOrZip);if($gen===null){return null;}foreach($gen as $stat){$t=$stat;if($predicate($t)===true){return $stat;}}return null;}if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
