...
namespace Inilim\Tool\Method\Zip{function findFirstResourceByCallable($pathToFileOrZip,callable $predicate){$zip=\Inilim\Tool\Method\Zip\getObjFrom($pathToFileOrZip);$find=\Inilim\Tool\Method\Zip\findFirstByCallable($zip,$predicate);if($find===null){return null;}return \Inilim\Tool\Method\Zip\getResourceByIdx($zip,$find['index']);}if(!\Inilim\Tool\Zip::__definedIfNot('findFirstByCallable')){
    function findFirstByCallable($pathToFileOrZip,callable $predicate):?array{foreach(\Inilim\Tool\Method\Zip\scanAsGenerator($pathToFileOrZip)as $stat){$t=$stat;if($predicate($t)===true){return $stat;}}return null;}
    }if(!\Inilim\Tool\Zip::__definedIfNot('getObjFrom')){
    function getObjFrom($pathToFileOrZip):\ZipArchive{\Inilim\Tool\Method\Assert\extPhp('zip');$type=\Inilim\Tool\Method\Other\getType($pathToFileOrZip);if($type==='string'){if(!\Inilim\Tool\Method\FS\isFile($pathToFileOrZip)){throw new \InvalidArgumentException(\sprintf('File "%s", not exist',$pathToFileOrZip));}$zip=\Inilim\Tool\Method\Zip\open($pathToFileOrZip);if(!$zip){throw new \InvalidArgumentException(\sprintf('File "%s", open failed',$pathToFileOrZip));}}elseif($type==='object'){if(!$pathToFileOrZip instanceof \ZipArchive){throw new \InvalidArgumentException(\sprintf('Expected (arg #0) a string or \ZipArchive. Got: %s',\get_class($pathToFileOrZip)));}$zip=$pathToFileOrZip;if($zip -> filename===''){throw new \InvalidArgumentException('Uninitialized zip');}}else{throw new \InvalidArgumentException(\sprintf('Expected (arg #0) a string or \ZipArchive. Got: %s',$type));}return $zip;}
    ...
    function scanAsGenerator($pathToFileOrZip):\Generator{$zip=\Inilim\Tool\Method\Zip\getObjFrom($pathToFileOrZip);$num=$zip -> numFiles;for($i=0;$i<$num;$i++){$ri=$zip -> statIndex($i,\ZipArchive :: FL_UNCHANGED);if($ri===false){continue;}yield $ri;}}
