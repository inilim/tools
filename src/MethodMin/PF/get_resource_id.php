...
namespace Inilim\Tool\Method\PF{function get_resource_id($res){if(\Inilim\Tool\Method\Check\php80()){return \get_resource_id($res);}if(!\is_resource($res)&&null===@\get_resource_type($res)){throw new \TypeError(\sprintf('Argument 1 passed to get_resource_id() must be of the type resource, %s given',\Inilim\Tool\Method\PF\get_debug_type($res)));}return (int) $res;}if(!\Inilim\Tool\PF::__definedIfNot('get_debug_type')){
