...
declare(strict_types=1);namespace Inilim\Tool\Method\PF{function ctype_alpha($text):bool{if(\Inilim\Tool\Method\Other\funcPhp('ctype_alpha')){return \ctype_alpha($text);}$cls=\Inilim\Tool\Method\PF\__resourceCache('convert_int_to_char_for_ctype');$text=$cls -> __invoke($text,'ctype_alpha');return \is_string($text)&&''!==$text&&!\preg_match('/[^A-Za-z]/',$text);}if(!\Inilim\Tool\PF::__definedIfNot('__resource')){
