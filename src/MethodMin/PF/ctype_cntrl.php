...
declare(strict_types=1);namespace Inilim\Tool\Method\PF{function ctype_cntrl($text):bool{if(\Inilim\Tool\Method\Other\funcPhp('ctype_cntrl')){return \ctype_cntrl($text);}$cls=\Inilim\Tool\Method\PF\__resourceCache('convert_int_to_char_for_ctype');$text=$cls -> __invoke($text,'ctype_cntrl');return \is_string($text)&&''!==$text&&!\preg_match('/[^\x00-\x1f\x7f]/',$text);}if(!\Inilim\Tool\PF::__definedIfNot('__resource')){
