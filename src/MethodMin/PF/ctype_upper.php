...
declare(strict_types=1);namespace Inilim\Tool\Method\PF{function ctype_upper($text):bool{if(\Inilim\Tool\Method\Other\funcPhp('ctype_upper')){return \ctype_upper($text);}$cls=\Inilim\Tool\Method\PF\__resourceCache('convert_int_to_char_for_ctype');$text=$cls -> __invoke($text,'ctype_upper');return \is_string($text)&&''!==$text&&!\preg_match('/[^A-Z]/',$text);}if(!\Inilim\Tool\PF::__definedIfNot('__resource')){
