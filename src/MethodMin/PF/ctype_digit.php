...
namespace Inilim\Tool\Method\PF{function ctype_digit($text):bool{if(\Inilim\Tool\Method\Other\funcPhp('ctype_digit')){return \ctype_digit($text);}$cls=\Inilim\Tool\Method\PF\__resourceCache('convert_int_to_char_for_ctype');$text=$cls -> __invoke($text,'ctype_digit');return \is_string($text)&&''!==$text&&!\preg_match('/[^0-9]/',$text);}if(!\Inilim\Tool\PF::__definedIfNot('__resource')){
