...
declare(strict_types=1);namespace Inilim\Tool\Method\FS{function missing(string $path):bool{return!\Inilim\Tool\Method\FS\exists($path);}if(!\Inilim\Tool\FS::__definedIfNot('exists')){
    function exists(string $filename):bool{$value=\Inilim\Tool\Method\Other\tryCallWithErrHandler_m2(static function()use($filename){\clearstatcache(false,$filename);return \file_exists($filename);});return \is_bool($value)?$value:false;}
