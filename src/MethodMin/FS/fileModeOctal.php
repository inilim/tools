...
namespace Inilim\Tool\Method\FS{function fileModeOctal(string $filename):?string{$value=\Inilim\Tool\Method\FS\filePerms($filename);return $value===null?null:\substr(\sprintf('%o',$value),-4);}if(!\Inilim\Tool\FS::__definedIfNot('filePerms')){
    function filePerms(string $filename):?int{$value=\Inilim\Tool\Method\Other\tryCallWithErrHandler(static fn()=>\fileperms($filename),null);return $value===false?null:$value;}
