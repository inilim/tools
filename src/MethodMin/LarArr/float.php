...
namespace Inilim\Tool\Method\LarArr{function float($array,$key,?float $default=null):float{$value=\Inilim\Tool\Method\LarArr\get($array,$key,$default);if(!\is_float($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be a float, %s found.',$key,\gettype($value)));}return $value;}if(!\Inilim\Tool\LarArr::__definedIfNot('accessible')){
