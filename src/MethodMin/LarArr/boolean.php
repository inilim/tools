...
namespace Inilim\Tool\Method\LarArr{function boolean($array,$key,?bool $default=null):bool{$value=\Inilim\Tool\Method\LarArr\get($array,$key,$default);if(!\is_bool($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be a boolean, %s found.',$key,\gettype($value)));}return $value;}if(!\Inilim\Tool\LarArr::__definedIfNot('accessible')){
