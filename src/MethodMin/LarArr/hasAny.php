...
namespace Inilim\Tool\Method\LarArr{function hasAny($array,$keys){if(\is_null($keys)){return false;}$keys=(array) $keys;if(!$array){return false;}if($keys===[]){return false;}foreach($keys as $key){if(\Inilim\Tool\Method\LarArr\has($array,$key)){return true;}}return false;}if(!\Inilim\Tool\LarArr::__definedIfNot('accessible')){
