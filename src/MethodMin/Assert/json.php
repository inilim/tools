...
declare(strict_types=1);namespace Inilim\Tool\Method\Assert{function json($value,string $message=''){\Inilim\Tool\Method\Assert\string($value);if(!\Inilim\Tool\Method\Check\isJson($value)){throw new \InvalidArgumentException(\sprintf($message?:'Expected an json. Got: "%s"',$value));}}if(!\Inilim\Tool\Assert::__definedIfNot('string')){
