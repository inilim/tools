...
namespace Inilim\Tool\Method\Assert{function integerish($value,string $message=''){if(!\is_numeric($value)||$value!=(int) $value){throw new \InvalidArgumentException(\sprintf($message?:'Expected an integerish value. Got: %s',\Inilim\Tool\Method\Other\getType($value)));}}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('getType')){
