...
declare(strict_types=1);namespace Inilim\Tool\Method\Assert{function isNonEmptyArray($value,string $message=''){\Inilim\Tool\Method\Assert\isArray($value);if(empty($value)){throw new \InvalidArgumentException($message?:'Expected a non-empty array.');}}if(!\Inilim\Tool\Assert::__definedIfNot('isArray')){
