...
namespace Inilim\Tool\Method\Assert{function httpHeaderName($value,string $message=''){\Inilim\Tool\Method\Assert\string($value,$message?:'Header name must be a string but %s provided.');if(!\Inilim\Tool\Method\Check\httpHeaderName($value)){throw new \InvalidArgumentException(\sprintf($message?:'"%s" is not valid header name.',$value));}}if(!\Inilim\Tool\Assert::__definedIfNot('string')){
