...
namespace Inilim\Tool\Method\Assert{function isCallable($value,string $message=''){if(!\is_callable($value)){throw new \InvalidArgumentException(\sprintf($message?:'Expected a callable. Got: %s',\Inilim\Tool\Method\Other\getType($value)));}}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('getType')){
