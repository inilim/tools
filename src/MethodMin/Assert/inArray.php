...
namespace Inilim\Tool\Method\Assert{function inArray($value,array $values,string $message=''){if(!\in_array($value,$values,true)){throw new \InvalidArgumentException(\sprintf($message?:'Expected one of: %2$s. Got: %s',\Inilim\Tool\Method\Other\valueToString($value),\implode(', ',\array_map('\Inilim\Tool\Method\Other\valueToString',$values))));}}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('funcPhp')){
