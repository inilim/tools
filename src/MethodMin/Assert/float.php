...
namespace Inilim\Tool\Method\Assert{function float($value,string $message=''){if(!\is_float($value)){throw new \InvalidArgumentException(\sprintf($message?:'Expected a float. Got: %s',\Inilim\Tool\Method\Other\getType($value)));}}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('getType')){
