...
namespace Inilim\Tool\Method\Assert{function notEq($value,$expect,string $message=''){if($expect==$value){throw new \InvalidArgumentException(\sprintf($message?:'Expected a different value than %s.',\Inilim\Tool\Method\Other\valueToString($expect)));}}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('funcPhp')){
