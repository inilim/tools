...
namespace Inilim\Tool\Method\Integer{function isBigIntUnsigned($value){if(!\Inilim\Tool\Method\Integer\isNumeric($value)){return false;}$value=\strval($value);if(\Inilim\Tool\Method\Str\_startsWith($value,'-')){return false;}$len=lenNumeric($value);if($len<\Inilim\Tool\Integer :: BIG_INT_MAX_UNSIGNED_LENGHT){return true;}if($len>\Inilim\Tool\Integer :: BIG_INT_MAX_UNSIGNED_LENGHT){return false;}return \Inilim\Tool\Method\Integer\__compare(\str_split($value),[1,8,4,4,6,7,4,4,0,7,3,7,0,9,5,5,1,6,1,5]);}if(!\Inilim\Tool\Integer::__definedIfNot('__compare')){
