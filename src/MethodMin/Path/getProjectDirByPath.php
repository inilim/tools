...
declare(strict_types=1);namespace Inilim\Tool\Method\Path{function getProjectDirByPath(?string $path=null):?string{$dir=\Inilim\Tool\Method\Path\getVendorDirByPath($path);return $dir?\dirname($dir,1):null;}if(!\Inilim\Tool\Path::__definedIfNot('getVendorDirByPath')){
    function getVendorDirByPath(?string $path=null):?string{if($path===null){$path=__DIR__;}else{$path=\Inilim\Tool\Method\Path\realPath($path);if($path===null){return null;}}$path=\Inilim\Tool\Method\Path\normalize($path.'/');if(\Inilim\Tool\Method\PF\str_contains($path,'/vendor/')){$t=\Inilim\Tool\Method\Str\beforeLast($path,'/vendor/');return \Inilim\Tool\Method\Path\normalize($t.'/vendor');}elseif(\Inilim\Tool\Method\PF\str_contains($path,'/src/')){$t=\Inilim\Tool\Method\Str\beforeLast($path,'/src/');$t=\Inilim\Tool\Method\Path\normalize($t.'/vendor');if(\is_dir($t)){return $t;}}$t=$path.'vendor';if(\is_dir($t)){return $t;}else{$t=\Inilim\Tool\Method\Path\normalize(\dirname($path).'/vendor');if(\is_dir($t)){return $t;}}return null;}
