...
namespace Inilim\Tool\Method\Enum{function in(object $enum,array $haystack):bool{\Inilim\Tool\Method\Assert\enumCase($enum);foreach($haystack as $item){if($enum===$item){return true;}}return false;}}namespace Inilim\Tool\Method\Other{if(!\Inilim\Tool\Other::__definedIfNot('getType')){
    ...
    function enumCase($value,string $message=''){if(!\Inilim\Tool\Method\Check\enumCase($value)){throw new \InvalidArgumentException(\sprintf($message?:'Expected an \UnitEnum. Got: %s',\Inilim\Tool\Method\Other\valueToString($value)));}}
    }}namespace Inilim\Tool\Method\Check{if(!\Inilim\Tool\Check::__definedIfNot('enumCase')){
