...
namespace Inilim\Tool\Method\File{function getInclude(string $pathToFile,array $data=[]){if(\is_file($pathToFile)){$__path=$pathToFile;$__data=$data;return(static function()use($__path,$__data){\extract($__data,\EXTR_SKIP);return include $__path;})();}throw \Inilim\Tool\Method\Obj\sprintfException('File does not exist at path "%s".',[$pathToFile]);}}namespace Inilim\Tool\Method\Obj{if(!\Inilim\Tool\Obj::__definedIfNot('rewriteLocationException')){
