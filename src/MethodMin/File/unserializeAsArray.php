...
namespace Inilim\Tool\Method\File{function unserializeAsArray(array $params){return \Inilim\Tool\Method\File\unserialize($params['pathToFile'],$params['options']??[],$params['lock']?? false,$params['throw']?? false,$params['default']?? null);}if(!\Inilim\Tool\File::__definedIfNot('get')){
    function get(string $pathToFile,int $offset=0,?int $length=null,bool $useIncludePath=false,bool $throw=false,$context=null,?array $contextParams=null):array{$args=['pathToFile'=>$pathToFile,'offset'=>$offset,'length'=>$length,'useIncludePath'=>$useIncludePath,'context'=>$context,'contextParams'=>$contextParams,'result'=>null,'exception'=>null,'errors'=>null];\Inilim\Tool\Method\Other\tryCallWithErrHandler(static function()use(&$args){if(\is_array($args['context'])){if($args['contextParams']===null){$args['context']=\stream_context_create($args['context']);}else{$args['context']=\stream_context_create($args['context'],$args['contextParams']);}}if($args['length']===null){$args['result']=\file_get_contents($args['pathToFile'],$args['useIncludePath'],$args['context'],$args['offset']);}else{$args['result']=\file_get_contents($args['pathToFile'],$args['useIncludePath'],$args['context'],$args['offset'],$args['length']);}},static function($type,$message,$file,$line)use(&$args){$args['errors']??=[];$args['errors'][]=[$message,$type,$file,$line];});if($args['errors']){$args['exception']=\Inilim\Tool\Method\Obj\getCollectionThrowable('File::get(...)');foreach($args['errors']as $err){[$message,$type,$file,$line]=$err;$args['exception'][]=new \ErrorException($message,$type,$type,$file,$line);}unset($args['errors']);}if($args['result']===false){if($throw&&$args['exception']){throw $args['exception'];}return['result'=>null,'exception'=>$args['exception']];}return['result'=>$args['result'],'exception'=>$args['exception']];}
    ...
    }if(!\Inilim\Tool\File::__definedIfNot('unserialize')){
    function unserialize(string $pathToFile,array $options=[],bool $lock=false,bool $throw=false,$default=null){if($lock){$data=\Inilim\Tool\Method\File\sharedGet($pathToFile,$throw);}else{$data=\Inilim\Tool\Method\File\get($pathToFile,0,null,false,$throw);}if($data['exception']){if($throw){throw $data['exception'];}return $default;}$data=$data['result'];if($data===''){return null;}if($data==='b:0;'){return false;}$errors=[];$undata=\Inilim\Tool\Method\Other\tryCallWithErrHandler(static function()use(&$data,&$options){return \unserialize($data,$options);},static function($type,$message,$file,$line)use(&$errors,$throw){if($throw){$errors[]=[$message,$type,$file,$line];}else{$errors[]=null;}});if($errors){if($throw){$e=\Inilim\Tool\Method\Obj\getCollectionThrowable();foreach($errors as $err){$e[]=new \ErrorException($err[0],$err[1],$err[1],$err[2],$err[3]);}throw $e;}return $default;}unset($data);if($undata===null||$undata===false){return $default;}return $undata;}
