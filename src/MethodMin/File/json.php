...
namespace Inilim\Tool\Method\File{function json(string $pathToFile,int $flags=0,bool $lock=false,bool $throw=false,$default=null){if($lock){$result=\Inilim\Tool\Method\File\sharedGet($pathToFile);}else{$result=\Inilim\Tool\Method\File\get($pathToFile);}if($result['exception']){if($throw){throw $result['exception'];}return $default;}if(!\Inilim\Tool\Method\Check\isJson($result['result'])){if($throw){throw new \JsonException(\sprintf('Content file not json "%s"',$pathToFile));}return $default;}return \json_decode($result['result'],true,512,$flags);}if(!\Inilim\Tool\File::__definedIfNot('get')){
