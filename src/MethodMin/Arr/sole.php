...
namespace Inilim\Tool\Method\Arr{function sole(array $array,?callable $callback=null){if($callback){$array=\Inilim\Tool\Method\LarArr\where($array,$callback);}$count=\sizeof($array);if($count===0){throw new \Exception('Item not found');}if($count>1){throw new \Exception('Multiple items found: '.$count);}return \Inilim\Tool\Method\LarArr\first($array);}}namespace Inilim\Tool\Method\Check{if(!\Inilim\Tool\Check::__definedIfNot('php84')){
    ...
    }}namespace Inilim\Tool\Method\LarArr{if(!\Inilim\Tool\LarArr::__definedIfNot('first')){
