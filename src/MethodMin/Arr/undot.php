...
namespace Inilim\Tool\Method\Arr{function undot($array):array{$results=[];$set=\Inilim\Tool\Method\LarArr\set();foreach($array as $key=>$value){$set($results,$key,$value);}return $results;}}namespace Inilim\Tool\Method\Assert{if(!\Inilim\Tool\Assert::__definedIfNot('__notArgsHere')){
    ...
    }}namespace Inilim\Tool\Method\LarArr{if(!\Inilim\Tool\LarArr::__definedIfNot('set')){
