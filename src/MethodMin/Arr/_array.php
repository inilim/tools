...
declare(strict_types=1);namespace Inilim\Tool\Method\Arr{function _array($array,$key,?array $default=null):array{$value=\Inilim\Tool\Method\Arr\get($array,$key,$default);if(!\is_array($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be an array, %s found.',$key,\gettype($value)));}return $value;}if(!\Inilim\Tool\Arr::__definedIfNot('accessible')){
