...
namespace Inilim\Tool\Method\Arr{function _array($array,$key,?array $default=null):array{$value=\Inilim\Tool\Method\LarArr\get($array,$key,$default);if(!\is_array($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be an array, %s found.',$key,\gettype($value)));}return $value;}}namespace Inilim\Tool\Method\Check{if(!\Inilim\Tool\Check::__definedIfNot('php80')){
