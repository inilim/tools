...
namespace Inilim\Tool\Method\Arr{function integer($array,$key,?int $default=null):int{$value=\Inilim\Tool\Method\Arr\get($array,$key,$default);if(!\is_int($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be an integer, %s found.',$key,\gettype($value)));}return $value;}if(!\Inilim\Tool\Arr::__definedIfNot('accessible')){
