...
namespace Inilim\Tool\Method\Arr{function string($array,$key,?string $default=null):string{$value=\Inilim\Tool\Method\LarArr\get($array,$key,$default);if(!\is_string($value)){throw new \InvalidArgumentException(\sprintf('Array value for key [%s] must be a string, %s found.',$key,\gettype($value)));}return $value;}}namespace Inilim\Tool\Method\Check{if(!\Inilim\Tool\Check::__definedIfNot('php80')){
