...
namespace Inilim\Tool\Method\Arr{function select(array $array,$keys){$keys=\Inilim\Tool\Method\LarArr\wrap($keys);return \Inilim\Tool\Method\LarArr\map($array,static function($item)use($keys){$result=[];foreach($keys as $key){if($key===null){continue;}if(\Inilim\Tool\Method\LarArr\accessible($item)&&\Inilim\Tool\Method\LarArr\exists($item,$key)){$result[$key]=$item[$key];}elseif(\is_object($item)&&isset($item ->{$key})){$result[$key]=$item ->{$key};}}return $result;});}}namespace Inilim\Tool\Method\LarArr{if(!\Inilim\Tool\LarArr::__definedIfNot('accessible')){
